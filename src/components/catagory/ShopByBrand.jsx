...
function ShopByBrand() {
    ...
    const filtereBrand = brands.filter(cat =>
        cat.brandName?.toLowerCase().includes(searchItem.toLowerCase())
    );
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex justify-end mb-6">
                {!loading && brands?.length > 0 && !showAll && (
                    <button
            ...
            ) : !filtereBrand || filtereBrand.length === 0 ? (
                <div className="w-full flex flex-col items-center justify-center py-12">
                    <p className="text-gray-500 text-sm mt-4">No brand found</p>
                </div>
            ) : showAll ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {filtereBrand.map((item, index) => (
                        <div 
                            key={getItemKey(item, index)}
                            onClick={() => {                               
                                navigate('/CatProduct', { state: { item } });
                              }}
                        ...
                        >
                            ...
                            <img
                                ...
                                className='w-25 h-25 object-contain transition-transform duration-300 hover:scale-105'
                                onError={(e) => {
                                    e.target.src = 'https://via.placeholder.com/80';
                                }}
                            />
                            <span className="text-xs md:text-sm text-center font-medium text-gray-700 line-clamp-2">
                                {item.brandName || 'Unnamed Category'}
                            </span>
