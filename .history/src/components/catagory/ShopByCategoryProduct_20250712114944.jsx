...
const ShopByCategoryProduct = () => {
    ...
    useEffect(() => {
        if (catProduct?.length > 0) {
            ...
            const uniqueBrands = [...new Set(catProduct.map(product => product.brand))];
            ...
            const uniqueManufacturers = [...new Set(catProduct.map(product => product.manufacturer))];
            setManufacturers(uniqueManufacturers.map(manu => ({ 
                id: manu?.toLowerCase()?.replace(/\s+/g, '') || '', 
                name: manu || '' 
            })));
        }
    }, [catProduct]);
    ...
    return (
        <div ref={topRef} className='bg-white w-full py-8'>
            <div className="flex flex-col md:flex-row gap-6 max-w-8xl mx-auto px-4 md:px-10 lg:px-20">
                ...
                <div
                    className={`${showMobileSidebar ? 'block' : 'hidden'} md:block w-full md:w-[20%] bg-white z-20 md:z-0 fixed md:sticky top-0 h-screen overflow-y-auto border border-gray-100 rounded-lg`}
                >
                            ...
                            <div className="mt-4">
                                ...
                                <div className="h-[180px] overflow-y-auto custom-scroll">
                                    {manufacturers.map((manu) => (
                                        <div key={manu.id} className="flex items-center py-1.5 px-3 hover:bg-gray-50 rounded">
                                            <input
                                                type="checkbox"
                                                id={manu.id}
                                            ...
                                            />
                                            <label htmlFor={manu.id} className="text-sm text-gray-600 whitespace-nowrap overflow-hidden text-ellipsis">
                                                {manu.name}
                                            </label>
                                        </div>
                                    ))}
